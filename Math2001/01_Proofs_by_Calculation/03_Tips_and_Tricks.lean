...
example {p : ℝ} (h1 : 5 * p - 3 = 3 * p + 1) : p = 2 := by
  have h2 : 2*p = 4 := by
    calc 2*p = (5 * p - 3)-(3 * p + 1) + 4 := by ring
          _  = (3 * p + 1) - (3 * p + 1) + 4 := by rw [h1]
          _  = 4 := by ring
  calc p = (2*p)/2 := by ring
      _  = 4/2 := by rw [h2]
      _ = 2 := by ring
  done

example {x y : ℤ} (h1 : 2 * x + y = 4) (h2 : x + y = 1) : x = 3 := by
  have h3 : y = 1-x := by addarith [h2]
  rw [h3] at h1
  calc x = (2 * x + (1 - x)) -1 := by ring
      _ = 4 - 1 := by rw [h1]
      _ = 3 := by ring
  done
...
example {x y : ℝ} (h1 : x + 3 = 5) (h2 : 2 * x - y * x = 0) : y = 2 := by
  have hx : x = 2 := by addarith [h1]
  rw [hx] at h2
  have hy : y*2 = 4 := by addarith [h2]
  calc y = (y*2)/2 := by ring
      _  = 4/2 := by rw [hy]
      _ = 2 := by ring
  done
...
example {p q r : ℚ} (h1 : p + q + r = 0) (h2 : p * q + p * r + q * r = 2) :
    p ^ 2 + q ^ 2 + r ^ 2 = -4 := by
    have H : p^2+q^2+r^2 = (p+q+r)^2 - 2*(p*q+p*r+q*r) := by ring
    rw [H]
    rw [h1,h2]
    ring
    done
